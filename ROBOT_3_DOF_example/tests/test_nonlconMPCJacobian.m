...
function tests = test_nonlconMPCJacobian
tests = functiontests(localfunctions);
end

%% Test Functions

function testnonlconMPCJacobian(testCase)
t1 = 0.1;
x1 = [1;1;1;1;1];
u0 = [.1;.1];
...
[~,~,actual,~] = nonlconMPC(t1, x1, u0, U, param);
addpath('./DERIVESTsuite');
[expected, err] = jacobianest(@(U) nonlconMPC(t1, x1, u0, U, param), U);
expected = expected.';
err = err.';
